...
static void mavlink_test_lidar_line(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	mavlink_lidar_line_t packet_in = {
		963497464,963497672,963497880,963498088,53
    };
	mavlink_lidar_line_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        	packet1.start_X = packet_in.start_X;
        	packet1.start_Y = packet_in.start_Y;
        	packet1.end_X = packet_in.end_X;
        	packet1.end_Y = packet_in.end_Y;
        	packet1.last_line = packet_in.last_line;
        
        

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_lidar_line_encode(system_id, component_id, &msg, &packet1);
	mavlink_msg_lidar_line_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_lidar_line_pack(system_id, component_id, &msg , packet1.start_X , packet1.start_Y , packet1.end_X , packet1.end_Y , packet1.last_line );
	mavlink_msg_lidar_line_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_lidar_line_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.start_X , packet1.start_Y , packet1.end_X , packet1.end_Y , packet1.last_line );
	mavlink_msg_lidar_line_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_lidar_line_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_lidar_line_send(MAVLINK_COMM_1 , packet1.start_X , packet1.start_Y , packet1.end_X , packet1.end_Y , packet1.last_line );
	mavlink_msg_lidar_line_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_roborescueV1(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_lidar_line(system_id, component_id, last_msg);
