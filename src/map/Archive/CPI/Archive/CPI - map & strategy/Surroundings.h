...
//! the strategy
//! \details A class that combines the functionality of both the Mapping 
//! and the strategy
...
public:
   ...
   Surroundings(CommandsMap & map, CommandsStrategy & strategy);
   ...
   const vector<point> & getCompletePath(
      const Point & start, 
      const Point & finish
   );
   ...
   const vector<point> & updatemap(Vector<Line> & map,const Point & position);
   ...
   //! \param[in] gyroscopeData contains the rosbee gyroscope data for the 
   //! strategy
   void sendRosbeeData(auto gyroscopeData);
   
private:
   //! \brief a reference to the CommandsMap class
   CommandsMap map;
   //! \brief a reference to the CommandsStrategy class
